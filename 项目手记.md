...
具体实现上，使用到了Raft节点内的以下变量：
1. rf.mu 用来锁定当前Raft节点，防止数据竞争
2. rf.currentTerm  用来判断收到的请求是否来自已经过时的candidate
...
思考一下为什么StartElection函数外层判断使用req.Term与rf.currentTerm判断，内层才判断resp.Term与rf.currentTerm